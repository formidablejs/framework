...
var _$Contextφ = require('./Context'/*$path$*/);
...
var _$versionφ2 = requireDefault$__(require('../Support/Helpers/version'/*$path$*/));
const settings = {
	config: null,
	console: null,
	environment: null,
...
class Application {
	[Ψ__init__]($$ = null){
		...
		this.handler = ($$ && (vφ = $$.handler) !== undefined) ? (vφ) : null;
		this.context = ($$ && (vφ = $$.context) !== undefined) ? (vφ) : _$Contextφ.Context;
	...
	*/
	constructor(root){
		...
		settings.console = new _$consoleφ.Application('Formidable Framework',_$versionφ2.default());
	...
	*/
	...
	prepare(){
		...
		this.handler = this.make(_$Handlerφ.default,[this.config]);
	...
	resolve(){
		...
		if (resolvers && (resolvers instanceof Array)) {
			...
			for (let iφ = 0, itemsφ = iter$__(resolvers), lenφ = itemsφ.length; iφ < lenφ; iφ++) {
				...
				resolver = ((φ = resolver.default) != null) ? (φ) : resolver;
