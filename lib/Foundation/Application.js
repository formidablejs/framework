	...
	async resolve(){
		...
		if (resolvers && (resolvers instanceof Array)) {
			...
			for (let $22 = 0, $23 = iter$__(resolvers), $25 = $23.length; $22 < $25; $22++) {
				...
				await this.bootResolver(resolver);
				$21.push(await this.registerResolver(resolver));
			};
			return $21;
		};
	}
	
	async bootResolver(resolver){
		
		return await (new resolver(this)).boot();
	}
	
	async registerResolver(resolver){
		
		return await (new resolver(this)).register();
	}
};
