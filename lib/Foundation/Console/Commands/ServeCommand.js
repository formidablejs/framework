...
class ServeCommand extends $1.Command {
	...
	get commandList(){
		...
		list.push(("" + this.runtime + " server" + this.ext + " -f -s -v"));
	...
	handle(){
		...
		if (this.option('dev',false) == false) {
			...
			if (process.platform === 'win32') {
				...
				return $5.spawnSync(sh,[shFlag,this.runtime,("server" + this.ext),...args],conf);
			};
			
			return $5.spawnSync(this.runtime,[("server" + this.ext),...args],conf);
		} else {
