...
class MaintenanceCommand extends $1.Command {
	...
	down(){
		...
		if ($3.existsSync(this.file)) {
			...
			return this.message('error',"Application is already down.");
		};
		...
		if ($3.existsSync(this.file)) {
			...
			return this.message('info',"Application is now in maintenance mode.");
		};
		
		return this.message('error','Failed to put application in maintenance mode.');
	}
	...
	up(){
		...
		if (!($3.existsSync(this.file))) {
			...
			return this.message('info',"Application is already up.");
		};
		...
		if ($3.existsSync(this.file)) {
			...
			return this.message('error',"Failed to bring application out of maintenance.");
		};
		
		return this.message('info',"Application is now live.");
	}
};
