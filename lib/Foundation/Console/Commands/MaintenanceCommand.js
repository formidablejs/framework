...
class MaintenanceCommand extends $1.Command {
	...
	down(){
		...
		if ($3.existsSync(this.file)) {
			...
			this.message('info',"Application is now in maintenance mode.");
	...
	up(){
		...
		if (!($3.existsSync(this.file))) {
			...
			this.message('info',"Application is already up.");
		...
		if ($3.existsSync(this.file)) {
			...
			this.message('error',"Failed to bring application out of maintenance.");
