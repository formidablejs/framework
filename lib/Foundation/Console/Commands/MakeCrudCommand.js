...
class MakeCrudCommand extends $2.Command {
	...
	async handle(){
		...
		await this.app.console().run(("make:migration " + tableName + " --table=" + name.toLowerCase() + " " + (this.option('schema') ? ('--schema=' + this.option('schema')) : '')));
		...
		if (this.option('type')) {
			...
			await this.app.console().run(("make:type " + typeName + " " + (this.option('schema') ? ('--schema=' + this.option('schema')) : '')));
		};
