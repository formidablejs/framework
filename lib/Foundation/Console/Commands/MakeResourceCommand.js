...
class MakeResourceCommand extends $1.Command {
	...
	handle(){
		...
		if ($3.existsSync(filePath)) {
			...
			return this.message('error',("" + (this.resource) + " already exists."));
		};
		...
		if ($3.existsSync(filePath)) {
			...
			return this.message('info',("" + (this.resource) + " created successfully."));
		};
		
		return this.message('error',("" + (this.resource) + " not created."));
	}
};
