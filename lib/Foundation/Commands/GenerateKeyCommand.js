...
class GenerateKeyCommand extends _$consoleφ.Command {
	...
	get props(){
		...
		return {
			env: _$consoleφ2.Prop.string().alias('e').description('Set the application key'),
			show: _$consoleφ2.Prop.boolean().description('Display the key instead of modifying files')
		};
	}
	...
	handle(){
		...
		if (this.option('show',false)) {
			...
			return this.write(("<fg:green>Application key [" + key + "] generated successfully.</fg:green>"));
		};
		
		this.updateEnv(key);
		
		return this.write("<fg:green>Application key set successfully.</fg:green>");
	}
