...
class PersonalAccessToken {
	...
	@param {object} data
	*/
	static create(name,id,table,abilities = ['*'],ttl = null,data = {}){
		var self = this;
		...
		return this.getDatabase().table('personal_access_tokens').insert({
			...
			payload: $2.default.encrypt(data),
			ttl: ttl
			...
			return await $11.default.sign(data,self.getEncrypter().key(),{
	...
	*/
	static async find(token,protocol = null){
		...
		token = await this.getDatabase().table('personal_access_tokens').where({id: this.getEncrypter().decrypt(decodedToken.id)}).first();
	...
	*/
	static async destroy(token){
		...
		return await this.getDatabase().table('personal_access_tokens').where({id: this.getEncrypter().decrypt(decodedToken.id)}).del();
	}
	...
	*/
	static async verify(token){
		...
		try {
			return await $11.default.verify(token,this.getEncrypter().key());
		} catch (e) { };
