	...
	*/
	static routes(config = {}){
		...
		if (login === true) {
			...
			_$Managerφ.default.post('login',[_$LoginControllerφ.default,'login']).middleware(['guest',_$BeforeLoginφ.default]).name('login');
		};
		
		if (register === true) {
			
			_$Managerφ.default.post('register',[_$RegisterControllerφ.default,'register']).middleware(['guest',_$BeforeRegisterφ.default]).name('register');
		};
		
		if (logout === true) {
			
			_$Managerφ.default.post('logout',[_$LogoutControllerφ.default,'logout']).middleware(['auth',_$BeforeLogoutφ.default]).name('logout');
		};
		
		if (email === true) {
			
			_$Managerφ.default.post('email/verify',[_$EmailVerificationControllerφ.default,'verify']).name('email.verify').middleware(['signed',_$BeforeVerifyφ.default]);
			_$Managerφ.default.post('email/resend',[_$EmailVerificationControllerφ.default,'resend']).name('email.resend').middleware([_$BeforeResendφ.default]);
		};
		
		if (password === true) {
			
			_$Managerφ.default.post('password/forgot',[_$PasswordControllerφ.default,'forgot']).middleware(['guest',_$BeforeForgotφ.default]).name('password.forgot');
			return _$Managerφ.default.post('password/reset',[_$PasswordControllerφ.default,'reset']).middleware(['signed','guest',_$BeforeResetφ.default]).name('password.reset');
		};
	}
};
