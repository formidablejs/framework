...
class SessionDriver extends $7.default {
		...
		if (!($9.default(this.request.request.cookies.remember))) {
			...
			if ($10.default(remember)) {
				...
				if (!($9.default(user))) {
					...
					const date = new Date();
					const ttl = new Date(date.valueOf() + this.config.get('session.lifetime',$1.ms('2 hours')));
					
					return await this.attempt('auth:session',user,ttl.valueOf());
				};
			};
		};
	...
	*/
	async authenticate(body){
		...
		await this.attempt('auth:session',user,ttl.valueOf());
