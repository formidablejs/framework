...
class Driver {
	...
	*/
	...
	@param {Object} user
	*/
	afterEmailVerified(user){
		
		if ($8.default(events.onEmailVerified)) {
			
			return events.onEmailVerified(this.request,this.reply,user,this.protocol,this.params);
		};
	}
	...
	*/
	...
	async verifyEmail(){
		...
		const results = await this.afterEmailVerified(response);
