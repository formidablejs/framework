...
var _$EmailNotVerifiedExceptionφ = requireDefault$__(require('../Exceptions/EmailNotVerifiedException'/*$path$*/));
var _$EmailVerifiedExceptionφ = requireDefault$__(require('../Exceptions/EmailVerifiedException'/*$path$*/));
...
class Driver {
	...
	*/
	...
	@param {Object} user
	*/
	async attempt(name,user){
		
		const token = await this.createPersonalAccessToken(name,user.id);
	...
	*/
	async getPersonalAccessToken(token = null){
		...
		return await _$PersonalAccessTokenφ.default.find((!(_$isEmptyφ.default(token))) ? token : this.request.bearerToken(),this.protocol);
	}
	...
	*/
	async usingPersonalAccessToken(token){
		...
		return await _$PersonalAccessTokenφ.default.using(token);
	}
	...
	*/
	afterRegistered(user){
		...
		if (_$isFunctionφ.default(events.onRegistered)) {
	...
	*/
	afterAuthenticated(user){
		...
		if (_$isFunctionφ.default(events.onAuthenticated)) {
	...
	afterSessionDestroyed(){
		...
		if (_$isFunctionφ.default(events.onSessionDestroyed)) {
	...
	*/
	async sendVerificationEmail(user){
		...
		if (this.getVerificationMailer()) {
			...
			return _$mailerφ.Mail.to(user.email).send(new (this.getVerificationMailer())(this.request,user));
		};
	}
	...
	*/
	async sendResetPasswordEmail(user,token){
		...
		if (this.getResetPasswordMailer()) {
			...
			return _$mailerφ.Mail.to(user.email).send(new (this.getResetPasswordMailer())(this.request,user));
		};
	}
	...
	async verifyEmail(){
		...
		const response = await _$Databaseφ.default.table(this.getProvider.table).where('email',email).whereNull('email_verified_at').update('email_verified_at',_$nowφ.default());
	...
	async requestEmailVerificationUrl(body = new Object){
		...
		if (user) {
			...
			if (user.email_verified_at) {
				...
				throw new _$EmailVerifiedExceptionφ.default('Email is already verified.');
			};
	...
	*/
	async requestForgotPasswordUrl(body = new Object){
		...
		if (user && !user.email_verified_at) {
			...
			throw new _$EmailNotVerifiedExceptionφ.default('Email is not verified.');
		};
		
		if (user) {
			
			await _$Databaseφ.default.table('password_resets').where('email',user.email).delete();
			
			
			const token = _$strRandomφ.default(60);
			
			const results = await _$Databaseφ.default.table('password_resets').insert({
				email: user.email,
				token: token
			});
	...
	*/
	async updatePassword(body = new Object){
		...
		const tokenRequest = await _$Databaseφ.default.table('password_resets').join(this.getProvider.table,'password_resets.email','=',("" + (this.getProvider.table) + ".email")).where('password_resets.token',token).where('password_resets.email',email).select(
			'users.id',
			'users.password'
		).first();
		...
		if (await _$Hashφ.default.check(body.password,tokenRequest.password)) {
		...
		const updatedPassword = await _$Databaseφ.default.table(this.getProvider.table).where('id',tokenRequest.id).update({
			password: await _$Hashφ.default.make(body.password)
		});
		...
		await _$Databaseφ.default.table('password_resets').where('email',email).delete();
	...
	*/
	async destroy(token = null,body = new Object){
		...
		return await _$PersonalAccessTokenφ.default.destroy((!(_$isEmptyφ.default(token))) ? token : this.request.bearerToken());
	}
	...
	*/
	async createPersonalAccessToken(name,id){
		...
		return await _$PersonalAccessTokenφ.default.create(name,id,this.getProvider.table);
	}
	...
	*/
	async createUser(body){
		...
		return _$Databaseφ.default.table(self.getProvider.table).insert({
			name: body.name,
			email: body.email,
			password: await _$Hashφ.default.make(body.password)
		},(_$Configφ.default.client == 'pg') ? ['id'] : null).then(async function([user]) {
			
			user = (typeof user === 'object' && user.hasOwnProperty('id')) ? user.id : user;
			
			return await _$Databaseφ.default.table(self.getProvider.table).where('id',user).first();
		});
	}
	...
	*/
	async findUser(body){
		...
		return await _$Databaseφ.default.table(this.getProvider.table).where('email',body.email).first();
	}
	...
	*/
	async verificationUrl(user){
		...
		const signature = await _$URLφ.default.temporarySignedRoute('email.verify','2h',null,{
			email: user.email
		});
	...
	*/
	async passwordResetUrl(user,token){
		...
		const signature = await _$URLφ.default.temporarySignedRoute('password.reset','15m',null,{
			email: user.email,
			token: token
		});
