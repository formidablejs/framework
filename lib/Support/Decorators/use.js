...
var $5 = requireDefault$__(require('../Helpers/isEmpty'/*$path$*/));
...
function αuse(target,key,descriptor){
	...
	descriptor.value = async function() {
		...
		await definition.forEach(function(object,key) {
			...
			if (object instanceof $2.default) {
				
				response = object.handle(request,key);
			} else if (object == $6.default) {
				...
				parsed.request = true;
			} else if ($3.default.isPrototypeOf(object) || $6.default.isPrototypeOf(object)) {
				...
				Object.keys(request).map(function(key) {
					...
					if ($5.default(response[key])) { return response[key] = request[key] };
				});
				...
				if ((validator.fails())) {
					...
					throw $7.default.withMessages(validator.errors.errors);
				};
			} else {
