...
class MultipartServiceResolver extends _$ServiceResolverφ.default {
	...
	saveTempFiles(){
		...
		return this.app.addHook('preHandler',async function(/**@type {FastifyReply}*/request) {
			...
			if (_$isEmptyφ.default(request._rawFiles)) { request._rawFiles = {} };
			
			resφ = [];
			for (let iφ = 0, itemsφ = iter$__(files), lenφ2 = itemsφ.length; iφ < lenφ2; iφ++) {
				let file = itemsφ[iφ];
				const fileObject = new _$Fileφ.default(_$withoutφ.default(file,['fields']));
				
				if (!(_$isEmptyφ.default(request.rawFiles[file.fieldname]))) {
					
					request.rawFiles[file.fieldname] = request.rawFiles[file.fieldname].push(fileObject);
				} else {
					
					request.rawFiles[file.fieldname] = (new _$FileCollectionφ.default).push(fileObject);
				};
	...
	unlinkTempFiles(){
		...
		return this.app.onResponse(function(response,/**@type {FormRequest}*/request) {
			...
			for (let oφ = request.request.rawFiles, iφ3 = 0, keysφ = Object.keys(oφ), lφ = keysφ.length, key, collection; iφ3 < lφ; iφ3++){
				key = keysφ[iφ3];collection = oφ[key];
				
				collection.each(function(/**@type {File}*/file) {
					
					if (_$fsφ.existsSync(file.path)) { return _$fsφ2.unlinkSync(file.path) };
				});
			};
