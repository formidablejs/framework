...
class Route {
	...
	*/
	static addRoute(verb,pattern,action){
		...
		if (typeof pattern !== 'string' || (_$isStringφ.default(pattern) && pattern.trim() == '')) {
		...
		routes.push({
			method: verb,
			path: _$Pathφ.default.clean(prefix,pattern.trim()),
	...
	*/
	static view(path,view,data = {},statusCode = null){
		...
		return this.get(path,function() {
			...
			return _$ViewResponseφ.default.make(view,data,(statusCode != null) ? statusCode : 200);
		});
	}
