...
var $5 = requireDefault$__(require('../Response/Response'/*$path$*/));
...
async function resolveResponse(response,request,reply,skipResolvers = false){
	...
	if (response instanceof $4.default) {
		
		return await response.handle(request,reply);
	} else if (response instanceof $3.default) {
		
		return await response.toJson(reply);
	} else if (response instanceof $6.default) {
		
		return await response.toView(request,reply);
	} else if (response instanceof $1.Mailable) {
		
		reply.header('content-type','text/html');
		return await response.render();
	} else if (response instanceof $5.default) {
