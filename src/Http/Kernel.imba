...
export default class Kernel
	...
	def hasRoutes router, config
		for route in Route.all!
			if isArray(route.action) || (isFunction(route.action) && !isClass(route.action)) || (isClass(route.action) && !isFunction(route.action)) || route.action.constructor.name === 'AsyncFunction'
